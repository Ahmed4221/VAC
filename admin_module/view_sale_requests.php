...
if($_SESSION['loggedIn'] and  ($_SESSION["UserType"]=="admin")){
        ...
        if(isset($_GET["ApprovalOfSale"])){
            
            $message = $_GET["ApprovalOfSale"];
...
<!-- saved from url=(0072)file:///Users/rafayabbas/Documents/Personal/VAC/see_order_individual.php -->
  ...
  <title>View Sale Requests</title>
                          ...
                          <div class="data-tables datatable-dark">
                              <div id="dataTable3_wrapper" class="dataTables_wrapper dt-bootstrap4 no-footer">
                                  ...
                                  <thead class="text-capitalize" style="background: linear-gradient(90deg, rgba(4,2,11,1) 0%, rgba(27,0,255,1) 54%, rgba(6,1,6,1) 97%);">
                                      <tr role="row">
                                          <th class="sorting_asc" tabindex="0" aria-controls="dataTable3" rowspan="1" colspan="1" style="width: 167px;" aria-label="Name: activate to sort column descending" aria-sort="ascending">Sale ID</th>
                                          <th class="sorting" tabindex="0" aria-controls="dataTable3" rowspan="1" colspan="1" style="width: 143px;" aria-label="Office: activate to sort column ascending">Product Barcode</th>
                                          <th class="sorting" tabindex="0" aria-controls="dataTable3" rowspan="1" colspan="1" style="width: 86px;" aria-label="Age: activate to sort column ascending">Vendor Name </th>
                                          <th class="sorting" tabindex="0" aria-controls="dataTable3" rowspan="1" colspan="1" style="width: 162px;" aria-label="Start Date: activate to sort column ascending">Start Date</th>
                                          <th class="sorting" tabindex="0" aria-controls="dataTable3" rowspan="1" colspan="1" style="width: 122px;" aria-label="salary: activate to sort column ascending">End Date </th>
                                          <th class="sorting" tabindex="0" aria-controls="dataTable3" rowspan="1" colspan="1" style="width: 122px;" aria-label="salary: activate to sort column ascending">Discount Percentage</th>
                                          <th class="sorting" tabindex="0" aria-controls="dataTable3" rowspan="1" colspan="1" style="width: 200px;" aria-label="salary: activate to sort column ascending">Approve / Disapprove</th>
                                      </tr>
                                  </thead>
                                  <tbody>
                                   <?php
                                      
                                      $fetchSaleRequestsQuery = "SELECT * FROM `SaleOnProducts` where Approved =  '0'";
                                      $results = mysqli_query($conn,$fetchSaleRequestsQuery);
                                        //   $followingdata = $res->fetch_assoc();
                                        while($followingdata = mysqli_fetch_assoc($results)){
                                           
                                          $output = '                                        
                                          <tr role="row" class="odd">
                                          <td tabindex="0" class="sorting_1">'.$followingdata['SaleID'].'</td>
                                          <td class="">'.$followingdata['Barcode'].'</td>
                                          <td class="">'.$followingdata['Vendor_id'].'</td>
                                          <td class="">'.$followingdata['StartDate'].'</td>
                                          <td class="">'.$followingdata['EndDate'].'</td>
                                          <td class="">'.$followingdata['DiscountPercentage'].'</td>

                                          <td class=""> <button class = "btn btn-success custom_button" style = "margin-right: 5px;" onClick = "approved(this.parentNode.parentNode.children[0].innerText)">  Approve  </button><button class = "btn btn-danger custom_button" onClick = "dissapproved(this.parentNode.parentNode.children[0].innerText)">Disapprove</button> </td>
                                      </tr>';
                                          echo $output;}
...
<script>
    ...
    function dissapproved(sale_id) {
        window.location.href = (window.location.href.substring(0, window.location.href.lastIndexOf("/") + 1)) + "sale_disapproved.php?SaleID=" + sale_id;
    }
